...
void measure::enter() {
	...
	Widget::theme->drawBox(tft, CHX, CHY, 300, 140, false);
	Widget::theme->drawBox(tft, CHX+5, CHY+5, 290, 130, true);  
...
void measure::update(uint16_t x, uint16_t y, uint8_t z) {    
	...
	if (millis() - period > 150) {
		...
		index++;
		chart.setData(index % 250, d / 3, machine->c1.switched, machine->c1.banned,
			machine->c2.switched, machine->c2.banned,
			machine->c3.switched, machine->c3.banned);

		// machine->TransitionTo(&setupState);     // TEST
		machine->c1.update(d);
	}
	...
	scroll.setValue(machine->screen);
	scroll.update(x, y, z);
	machine->screen = scroll.getValue();
}
