...
/* LG4345 TFTdriver port to Maple Mini 
 * just for test, it's not a complet port
 */
...
/*
 * Adafruit TFT LCD constructor 
 */
  Adafruit_TFTLCD::Adafruit_TFTLCD(uint8_t rd, uint8_t wr, uint8_t cd, uint8_t cs, uint8_t rst)
    :  Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {
  
    /*
     * volatile uint32 *csport, *cdport, *wrport, *rdsport, *rstport;                 // pointer the specific port for PINs
     uint32_t  _cs, _cd, _wr, _rd, _rst;
     uint32_t  csmask, cdmask, wrmasl, rdmask, rstmask;
    ...
    *csport |=  csmask;
    *wrport |=  wrmask;
    *rdport |=  rdmask;            //  *csport &= ~cspinmask;  delete pin
  ...
  void Adafruit_TFTLCD::init(void) {
    ...
    CS_IDLE; // Set all control bits to idle state  !!! this macros are defined in pin magic h
    WR_IDLE;
    RD_IDLE;
    CD_DATA;
    digitalWrite(_rst, HIGH);           // Reset line
 ...
 void Adafruit_TFTLCD::test(void) {
   for ( int i=0; i  < 8; i++ ) {
    write8inline (  1 << i );
    delay ( 500 );
   }
...
void Adafruit_TFTLCD::drawFastBMP2 ( int16_t x0, int16_t y0, const uint16_t * mem, int16_t w, int16_t h ) 
{
  setAddrWindow(x0, y0, x0+w-1, y0+(h*2)-1); 
  uint16_t color;
  uint8_t  hi, lo;
  uint16_t len = w*h;
